...
// SVG Ikonkalar kutubxonasi (Yuklanish tezligini oshirish va tashqi kutubxonalarga bog'lanmaslik uchun)
const SVG_ICONS = {
  // Socials (Fill)
...
const server = http.createServer((req, res) => {
  // Faqat asosiy sahifani render qilamiz
  if (req.url === '/' || req.url === '/index.html') {
    const configPath = path.join(__dirname, 'config.json');
    const templatePath = path.join(__dirname, 'index.html');

    // config.json va index.html shablonini parallel ravishda o'qiymiz
    fs.readFile(configPath, 'utf8', (err, configData) => {
      if (err) {
        res.writeHead(500, { 'Content-Type': 'text/plain; charset=utf-8' });
        res.end('Tizim xatoligi: config.json faylini o‘qib bo‘lmadi.');
        return;
      }
      ...
      fs.readFile(templatePath, 'utf8', (err, htmlTemplate) => {
        if (err) {
          res.writeHead(500, { 'Content-Type': 'text/plain; charset=utf-8' });
          res.end('Tizim xatoligi: HTML shablon fayli topilmadi.');
          return;
        }
        ...
        try {
          ...
          // 1. Socials piktogrammalarini generatsiya qilish
          let socialsHtml = '';
          ...
          // 2. Havolalar ro'yxatini generatsiya qilish (faqat active: true bo'lganlari)
          let linksHtml = '';
          ...
          // 3. Shablondagi placeholder'larni haqiqiy qiymatlar bilan almashtirish
          ...
          res.writeHead(500, { 'Content-Type': 'text/plain; charset=utf-8' });
          res.end('Tizim xatoligi: config.json faylining formati noto‘g‘ri.');
        }
      });
    });
  } else {
...
server.listen(PORT, () => {
  console.log(`\n🚀 Personal Link Hub serveri muvaffaqiyatli ishga tushdi!`);
  console.log(`👉 Manzil: http://localhost:${PORT}`);
  console.log(`🛑 Serverni to'xtatish uchun: Ctrl + C tugmalarini bosing.\n`);
});
